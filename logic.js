...
function updateTotalExpense() {
    ...
    if (!totalExpenseElement) {
        console.error('Element with id "totalExpense" not found.');
        return;
    }

    const total = expenses.reduce((sum, expense) => sum + expense.amount, 0);
    totalExpenseElement.innerText = `₹${total.toFixed(2)}`;
}
...
function displayPayments() {
    ...
    if (!paymentsList) {
        console.error('Element with id "paymentsList" not found.');
        return;
    }
    ...
    payments.forEach(payment => {
        ...
        paymentItem.textContent = `${payment.payer} paid ₹${payment.amount} to ${payment.payee}`;
